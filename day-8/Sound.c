...
typedef enum _MOD_UPDATE_FLAGS
{
	...
	MOD_UPD_FLG_SET_FREQ	= BIT01,	// Row or non-row, recalculates mix chn increment
...
void SndInit(SND_FREQ freq)
{
	...
	sndVars.mixFreqPeriod	= div(AMIGA_VAL<<9, sndVars.mixFreq) << (MIXCHN_FRAC_BITS-9);
...
void SndUpdate()
{
	...
	while(samplesLeft > 0)
	{
		...
		if(sndVars.samplesUntilMODTick < (1<<12) && sndMod.state == MOD_STATE_PLAY)
		{
		...
		if((sndVars.samplesUntilMODTick>>12) < samplesLeft && sndMod.state == MOD_STATE_PLAY)
		{
