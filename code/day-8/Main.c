...
// 01/08/05 - (Day 6) Replaced the old control text, since those
//            buttons didn't even do anything anymore :-P
// 05/05/05 - (Day 7) Changed button assignments.
// 05/06/05 - (Day 8) Added a couple more songs.
// ----------------------
...
int AgbMain() {
  ...
  while (1) {
    ...
    FontPrint(0, 1, BGS(31), "D-pad: Songs, L/R: SFX");
    ...
    if (KEYPRESS(KEY_RIGHT)) {
      SndPlayMOD(MOD_FANTASY);
    }
    if (KEYPRESS(KEY_UP)) {
      SndPlayMOD(MOD_monday);
    }
    if (KEYPRESS(KEY_DOWN)) {
      SndPlayMOD(MOD_realtetris);
    }

    if (KEYPRESS(KEY_L)) {
      SndPlaySFX(SFX_MILITARY);
    }
    if (KEYPRESS(KEY_R)) {
      SndPlaySFX(SFX_PIZZICATO);
    }
