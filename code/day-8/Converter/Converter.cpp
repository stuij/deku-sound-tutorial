...
// 05/05/05 - Fixed invalid samples being output as dSmpData-1.
//			  Fixed loop length bug, where very short loop lengths are 
//			  appearently supposed to be treated as 0.
// ----------------------
    ...
    if (strchr(globals.modHeader[globals.modCount].name, MOD_SFX_FLAG)) {
      ...
      for (i = 0; i < 31; i++) {
        if (globals.modHeader[globals.modCount].sample[i].length > 0) {
          ...
          *globals.sfx[globals.sfxCount] =
              globals.modHeader[globals.modCount].sample[i];
          globals.sfxCount++;
        }
      }
...
void LoadMod(MOD_HEADER *modHeader, char *fileName) {
  ...
  modHeader->name[19] =
      '\0'; // Chop the last char to make sure string is NULL-terminated
...
void LoadSamples(MOD_HEADER *modHeader, FILE *modFile) {
  ...
  for (i = 0; i < 31; i++) {
    ...
    smp->loopLength =
        (((smp->loopLength & 0xff) << 8) | (smp->loopLength >> 8));
    ...
    for (row = 0; row < 64; row++) {
      for (column = 0; column < 4; column++) {
        ...
        if (period == 0) {
          ...
          // Now we'll loop through our note period table and find the closest
          // match
          u16 closestDist =
              0xffff; // make sure the first comparison sets the closest note
          closestNote =
              0; // But still initialize note to prevent compiler warning
...
void LoadSampleDatas(MOD_HEADER *modHeader, FILE *modFile) {
  ...
  for (curSmp = 0; curSmp < 31; curSmp++) {
    if (modHeader->sample[curSmp].length > 0) {
      ...
      for (i = 0; i < globals.smpDataCount; i++) {
        if (tempSmpData.length == globals.smpData[i].length) {
          if (!memcmp(tempSmpData.data, globals.smpData[i].data,
                      tempSmpData.length)) {
    ...
    // Else leave this sample's smpDataIdx as invalid
  }
...
void WriteCFile(const char *fileName) {
  ...
  for (curMod = 0; curMod < globals.modCount; curMod++) {
    ...
    // Find the last sample non-zero length sample, or stop at 0 if none are
    // used
    for (i = 30; i > 0 && globals.modHeader[curMod].sample[i].length == 0;
         i--) {
    }
  ...
  if (globals.sfxCount == 0) {
    ...
    for (i = 0; i < globals.sfxCount; i++) {
      fprintf(outFile, "\n\t{ %i, %i, %i, %i, %i, dSmpData%i }, ",
              globals.sfx[i]->length, globals.sfx[i]->finetune,
              globals.sfx[i]->vol, globals.sfx[i]->loopStart,
              globals.sfx[i]->loopLength, globals.sfx[i]->smpDataIdx);
    }
  }
  fprintf(outFile, "\n};\n");
...
void WriteAsmFile(const char *fileName) {
  ...
  // Patterns
  for (curMod = 0; curMod < globals.modCount; curMod++) {
    for (curPattern = 0; curPattern < globals.modHeader[curMod].patternCount;
         curPattern++) {
      ...
      for (i = 0; i < 1024; i++) {
        ...
        fprintf(outFile, "%i",
                globals.modHeader[curMod].pattern[curPattern][i]);
      }
