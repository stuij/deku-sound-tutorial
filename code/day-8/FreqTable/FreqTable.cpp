...
int main(int argc, char **argv) {
    ...
    for (octave = 0; octave < 5; octave++) {
      for (note = 0; note < 12; note++) {
        ...
        /*			 u32 tempFreq   = (AMIGA_VAL / tempPeriod);
                                        ...
                                        // Clip to 16 bits, because only the
           highest notes on the
                                        // highest finetunes will overflow it,
           and they're not worth
                                        // using u32's and doubling the size of
           the table if(tempFreq > 65535) tempFreq = 65535;*/

        freqTable[finetune * 12 * 5 + octave * 12 + note] = (u16)tempPeriod;
      }
    }
  }
  ...
  fprintf(file, "const u16 notePeriodTable[] =\n{");
  for (i = 0; i < 16 * 5 * 12; i++) {
